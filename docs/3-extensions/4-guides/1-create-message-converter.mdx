...
To visualize GPS coordinates, the [Map panel](/visualization/panels/map) requires messages in the [`foxglove.LocationFix`](/visualizing/supported-messages/location-fix) format. In short, our converter needs to turn `MyGps` messages into messages that adhere to the `foxglove.LocationFix` schema.
...
Once we've packaged and installed our extension, load any data source containing `sensors.MyGps` messages in Foxglove and visualize them with the [Map panel](/visualization/panels/map).
