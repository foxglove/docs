...
While some Foxglove panels (e.g. [Raw Messages](/docs/studio/panels/raw-messages) and [Plot](/docs/studio/panels/plot)) can display data of any type from any topic, others (e.g. [Map](/docs/studio/panels/map) and [3D](/docs/studio/panels/3d)) require incoming messages to conform to a known schema to visualize them properly.

To transform custom messages into supported Foxglove schemas, you can write a [message converter](/docs/studio/extensions/getting-started#message-converters).

See the [panels](TBD) for each panel's supported messages.

## Using Foxglove schemas

Adhering to supported schemas will help you take full advantage of Foxglove's built-in visualizations.

Use the schema files in [`@foxglove/schemas`](https://github.com/foxglove/schemas) to write structured messages in any of the following formats:
...
In Foxglove's [User Scripts panel](/docs/studio/panels/user-scripts), you can specify the schema you want with `Message<"foxglove.[SchemaName]">`:
...
Import these types when working on a JavaScript WebSocket or [MCAP](https://mcap.dev) project, or when writing a custom data transformation script in Foxglove's [User Scripts panel](/docs/studio/panels/user-scripts).
