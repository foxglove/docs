...
export const meta = {
  ...
  heroImage: "/img/docs/studio/app-concepts/playback/bar.webp",
};
...
Both Foxglove Studio and the [Foxglove WebSocket protocol](https://github.com/foxglove/ws-protocol/blob/main/docs/spec.md) play messages in receive time, or `log_time`, order.

### Recorded data

Though all Foxglove Studio [panels](/docs/studio/panels/introduction) order messages by receive time, some – like the [Plot](/docs/studio/panels/plot) and [State Transitions](/docs/studio/panels/state-transitions) panels – can be configured to order messages by header stamp as well.
...
Navigate to the [Foxglove console](https://console.foxglove.dev)’s [“Recordings” page](https://console.foxglove.dev/recordings) and select a data recording to “View in Studio”.
...
Open the Data source sidebar and click on the “Events” tab to see all your events. The search input field at the top of the tab uses the [same query syntax supported in Foxglove Data Platform](/docs/data-platform/events#search-events). Type in some queries to filter events by metadata keys and values and find events of interest.
