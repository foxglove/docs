...
Certain panels like the [3D](/visualization/panels/3d) and [Variable Slider](/visualization/panels/variable-slider) panels have built-in ways to update variable values with user interactions. Panels that support [message path syntax](/visualizing/message-path-syntax) – like [Raw Messages](/visualization/panels/raw-messages), [Plot](/visualization/panels/plot), and [State Transitions](/visualization/panels/state-transitions) – can reference variables to dynamically decide what to visualize.
...
- Type `/my_objects.objects[:]{id==$my_ID}` in a [Raw Messages panel](/visualization/panels/raw-messages) to inspect the object whose `id` field equals `101`
- Add `/my_objects.objects[:]{id==$my_ID}.velocity` as a y-axis value in a [Plot panel](/visualization/panels/plot) to plot that same object's `velocity`
