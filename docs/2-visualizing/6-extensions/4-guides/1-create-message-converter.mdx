...
To visualize GPS coordinates, the [Map panel](/docs/visualization/panels/map) requires messages in the [`foxglove.LocationFix`](/docs/visualizing/supported-messages/location-fix) format. In short, our converter needs to turn `MyGps` messages into messages that adhere to the `foxglove.LocationFix` schema.
