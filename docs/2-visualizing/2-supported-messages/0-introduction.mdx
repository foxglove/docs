...
If you've already written custom messages, you can transform them into Foxglove-supported schemas using a [message converter](/docs/visualizing/extensions/getting-started#message-converters) extension.
...
_NOTE: For Protobuf data, time values of type [`google.protobuf.Timestamp`](https://protobuf.dev/reference/protobuf/google.protobuf/#timestamp) or [`google.protobuf.Duration`](https://protobuf.dev/reference/protobuf/google.protobuf/#duration) will appear with `sec` and `nsec` fields (instead of `seconds` and `nanos`) in [user scripts](/docs/visualization/panels/user-scripts), [message converters](/docs/visualizing/extensions/getting-started#message-converters), and the rest of Foxglove, for consistency with time and duration types in other data formats._
