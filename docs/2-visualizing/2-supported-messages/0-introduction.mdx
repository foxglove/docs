...
If you've already written custom messages, you can transform them into Foxglove-supported schemas using a [message converter](/extensions/getting-started#message-converters) extension.

### Protobuf and JSON Schema

Copy the [`.proto` files](https://github.com/foxglove/schemas/tree/main/schemas/proto/foxglove) or [`.json` files](https://github.com/foxglove/schemas/tree/main/schemas/jsonschema) you need directly into your project, and use them to start publishing data via a [live Foxglove WebSocket connection](/connecting-to-data/frameworks/custom#foxglove-websocket) or logging data to an [MCAP file](https://mcap.dev).

_NOTE: For Protobuf data, time values of type [`google.protobuf.Timestamp`](https://protobuf.dev/reference/protobuf/google.protobuf/#timestamp) or [`google.protobuf.Duration`](https://protobuf.dev/reference/protobuf/google.protobuf/#duration) will appear with `sec` and `nsec` fields (instead of `seconds` and `nanos`) in [user scripts](/visualization/panels/user-scripts), [message converters](/extensions/getting-started#message-converters), and the rest of Foxglove, for consistency with time and duration types in other data formats._
...
In Foxglove's [User Scripts panel](/visualization/panels/user-scripts), you can specify the schema you want with `Message<"foxglove.[SchemaName]">`:
...
Import these types when working on a JavaScript WebSocket or [MCAP](https://mcap.dev) project, or when writing a custom data transformation script in Foxglove's [User Scripts panel](/visualization/panels/user-scripts).
