...
![autocomplete options](/img/docs/visualizing/message-path-syntax/autocomplete.webp)
...
Slice on a [variable](/docs/visualizing/variables) by prepending each variable name with a `$`. For example, if you've set `$my_start_idx` to 3 and `$my_end_idx` to 4:
...
In addition to filtering on primitive values, you can filter on a [variable](/docs/visualizing/variables):
