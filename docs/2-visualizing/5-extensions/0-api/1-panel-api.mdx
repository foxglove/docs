...
export const meta = {
  ...
  description: "Use the PanelExtensionContext to implement your custom panel.",
};
...
The more common methods and properties are described below. For full type docs on the `PanelExtensionContext` and `RenderState` types, see [`@foxglove/studio`](https://github.com/foxglove/studio/blob/main/packages/studio/index.d.ts).

> The `initPanel` function used in the [`registerPanel` method](/docs/studio/extensions/extension-context#registerpanel) accepts a `PanelExtensionContext` argument. This argument contains properties and methods for accessing panel data and rendering UI updates. The `initPanel` function also returns an optional cleanup function to run when the extension `panelElement` unmounts.
...
### [Native (ROS 1)](/docs/studio/connection/ros1#native)
...
### [Foxglove WebSocket](/docs/studio/connection/custom#foxglove-websocket)

`options` depend on the server implementation.

#### [Foxglove Bridge](/docs/studio/connection/using-foxglove-bridge)
...
### [Rosbridge](/docs/studio/connection/rosbridge)
...
Use `context.setVariable` to set a [variable](/docs/studio/app-concepts/variables) `name` to any valid variable `value`.
