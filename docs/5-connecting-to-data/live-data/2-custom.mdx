...
Foxglove Studio can load your custom data (i.e. [Protobuf](https://developers.google.com/protocol-buffers), [JSON](http://jsonschema.org/), [FlatBuffers](https://google.github.io/flatbuffers)) as local or remote [MCAP](https://mcap.dev) files. It can also connect directly to your live custom data with the Foxglove WebSocket, an encoding-agnostic WebSocket connection.

## Files

Load custom data into Foxglove Studio by first encoding it in [MCAP](https://mcap.dev) (`.mcap`) files.
...
Once you have your local MCAP file, follow the MCAP directions for [loading it in Studio](/docs/studio/connection/mcap#local).

### Cloud data

#### Foxglove Data Platform

Once you've [recorded your custom data to MCAP (`.mcap`) files](/docs/studio/connection/custom#local-file), you can upload them to [Foxglove Data Platform](/data-platform)'s [web console](https://console.foxglove.dev) and stream them directly into Foxglove Studio.
...
Once you have an MCAP file in remote storage, follow the MCAP directions for [opening it via URL in Studio](/docs/studio/connection/mcap#remote).
...
The [`@foxglove/ws-protocol-examples`](https://github.com/foxglove/ws-protocol/tree/main/typescript/ws-protocol-examples) npm package also provides example servers and clients that you can run to see how Studio can receive system stats and image data from a custom WebSocket server:
...
Studio expects the schema data to be a binary [FileDescriptorSet](https://developers.google.com/protocol-buffers/docs/techniques#self-description). For Foxglove WebSocket connections, the schema must additionally be base64-encoded because it is represented as a string.
...
Each message should be encoded in the [Protobuf binary wire format](https://developers.google.com/protocol-buffers/docs/encoding) – we provide `.proto` files compatible with Foxglove Studio in our [`foxglove/schemas` repo](https://github.com/foxglove/schemas).
...
Studio expects the schema data to be a concatenation of the referenced `.msg` or `.idl` file and its dependencies. For more information about the concatenated format, see the [MCAP specification appendix](https://mcap.dev/specification/appendix.html#ros1msg-data-format).
...
Known limitations can be found here in `foxglove/omgidl` library internally used by Studio:
 ...
 If there's a feature of IDL that you want to see supported, please file an issue on the [Foxglove Studio repo](https://github.com/foxglove/studio/issues) or the [Foxglove OMG IDL repo](https://github.com/foxglove/omgidl/issues).
...
- [Use FlatBuffers data in Foxglove Studio](/blog/announcing-flatbuffers-support-in-foxglove)
