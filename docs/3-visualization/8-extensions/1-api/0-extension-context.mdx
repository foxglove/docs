...
The converter function is invoked when a panel subscribes to a topic with the [convertTo](/docs/visualization/extensions/api/panel-api#message-converters) option indicating it would like to run the converter on the original message. The return value of the converter function is the converted message and is provided to the panel.
