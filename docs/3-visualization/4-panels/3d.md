...
There's no way to check this in our desktop app yet, however, because our desktop app uses Electron (a Chromium-based framework), if you see that WebGL acceleration is not present in Chrome, there's a good chance it's not present in our application for some of the same reasons. So we recommend getting things working in Chrome first, and if the desktop app is still having issues afterward then please file and issue and we'll look into it.
