...
If you are experiencing poor performance interacting with the 3D panel, the first thing to check is that `webgl` hardware acceleration is enabled on your platform. Hardware acceleration can drastically increase the performance of the 3D panel by doing rendering on the GPU rather than the CPU. In your browser, you can check that hardware acceleration is enabled by going to `chrome://gpu` and seeing that "WebGL" and/or "WebGL2" are "Hardware accelerated".

There's no way to check this in our desktop app yet. However, because our desktop app uses Electron (a Chromium-based framework), if you see that WebGL acceleration is not present in Chrome, there's a good chance it's not present in our application for the same reasons. So we recommend getting things working in Chrome first, and if the desktop app is still having issues afterward then please file an issue and we'll look into it.

To enable WebGL hardware acceleration in your browser you can go to `chrome://settings`, search your settings for `acceleration` and ensure that the "Use graphics acceleration when available" toggle is enabled.

If you still see that WebGL is not "Hardware accelerated" in `chrome://gpu` then you might have to take more platform-specific steps for enabling this with your GPU.
