...
Note, if referring to a device by name, that the device must be known to Foxglove in advance. If you supply a device name which doesn't have an associated Foxglove ID, your import will fail. To add the device to Foxglove, see [_Add a device_](/docs/data-platform/importing-data#add-a-device).
