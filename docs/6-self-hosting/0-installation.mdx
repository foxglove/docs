...
---
  ...
  description: "Install and configure your Primary Site.",
...
> Find your Primary Site's site token on the _Sites_ tab of Foxglove web console's [_Settings_ page](https://console.foxglove.dev/settings/sites).
...
Create a `values.yaml` file to configure the installation. You'll need the names of your `lake` and 'inbox' buckets.
...
Accepted values for `storageProvider` are: `aws`, `azure`, or `google_cloud`.
...
> Note: Helm will install the Primary Site deployment to your current Kubernetes [context](https://kubernetes.io/docs/concepts/configuration/organize-cluster-access-kubeconfig/#context)
...
You can completely disable the ingress by setting `ingress.enabled` to false. However, you must provide an Ingress for the site to function.
...
Once the site's ingress public URL is known, you can configure it on the Site details page, linked from Foxglove
web console's [_Settings_ page](https://console.foxglove.dev/settings/sites).

Under **Site URL**, enter the HTTPS URL you've assigned to the Ingress.

## 8. Bucket push notification
...
> Org admins can find the inbox notification endpoint on the Foxglove web console's [_Settings_ page](https://console.foxglove.dev/settings/sites).

## Next steps

Congratulations! 🦊 Your Primary Site is setup and ready for use.
