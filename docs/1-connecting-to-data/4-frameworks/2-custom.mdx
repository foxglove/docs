...
_NOTE: For Protobuf data, time values of type [`google.protobuf.Timestamp`](https://protobuf.dev/reference/protobuf/google.protobuf/#timestamp) or [`google.protobuf.Duration`](https://protobuf.dev/reference/protobuf/google.protobuf/#duration) will appear with `sec` and `nsec` fields (instead of `seconds` and `nanos`) in [user scripts](/docs/visualizing/panels/user-scripts), [message converters](/docs/visualizing/extensions/getting-started#message-converters), and the rest of Foxglove, for consistency with time and duration types in other data formats._
...
We provide `.idl` files compatible with Foxglove in our [`foxglove/schemas` repo](https://github.com/foxglove/schemas/tree/main/schemas/omgidl).
