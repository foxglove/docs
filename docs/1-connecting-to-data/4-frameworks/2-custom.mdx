...
Once you have an MCAP file in remote storage, follow the MCAP directions for [opening it via URL in Foxglove](/connecting-to-data/frameworks/mcap#remote-file).
...
_NOTE: For Protobuf data, time values of type [`google.protobuf.Timestamp`](https://protobuf.dev/reference/protobuf/google.protobuf/#timestamp) or [`google.protobuf.Duration`](https://protobuf.dev/reference/protobuf/google.protobuf/#duration) will appear with `sec` and `nsec` fields (instead of `seconds` and `nanos`) in [user scripts](/visualizing/panels/user-scripts), [message converters](/extensions/getting-started#message-converters), and the rest of Foxglove, for consistency with time and duration types in other data formats._
...
If there's an IDL feature you want to see supported, file an issue on the [Foxglove](https://github.com/foxglove/studio/issues) or [Foxglove OMG IDL](https://github.com/foxglove/omgidl/issues) GitHub repo.
