...
Foxglove Studio can load local and remote [ROS 1](/tutorials/installing-ros1-on-ubuntu) (`.bag`) files, or connect directly to a running ROS stack using a Rosbridge (WebSockets) or native (TCP) connection.
...
Use [Foxglove Data Platform](/data-platform)'s [web console](https://console.foxglove.dev) to upload your ROS 1 `.bag` files and stream it directly into Foxglove Studio.
...
If you're running ROS and Studio on different machines see the ROS 1 [Network Setup](http://wiki.ros.org/ROS/NetworkSetup) docs to make sure your environment is configured correctly.
