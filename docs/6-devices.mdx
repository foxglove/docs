...
Add, list, delete, and search devices from the [Devices page](https://app.foxglove.dev/~/devices).
...
Admins can go into the [Custom properties settings](https://app.foxglove.dev/~/settings/custom-properties) to create custom properties for their organization's devices.
...
After assigning custom property values to various resources, go to [Devices](https://app.foxglove.dev/~/devices) to query by those values (e.g. devices with a `version` set to 65).
